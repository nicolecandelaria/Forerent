    ...
    <div class="flex flex-col lg:flex-row gap-6 w-full">
        ...
        <div class="w-full lg:w-[30%] flex-shrink-0 h-[750px] bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden p-2">
            ...
            <div class="p-4 pb-3 border-b border-gray-50 flex-shrink-0">
                <h3 class="text-xl font-bold text-[#070642] mb-3">My Violations</h3>

                <div class="relative">
                    <input
                        type="text"
                        placeholder="Search by violation # or category..."
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-2.5" style="scrollbar-width: thin; scrollbar-color: #e2e8f0 transparent;">
                @forelse($violations as $vio)
                    @php
                        ...
                        $penaltyShort = match($vio->penalty_type) {
                ...
                @empty
                    <div class="flex flex-col items-center justify-center h-full text-gray-400 py-16">
                        @if(!empty($search) || $activeTab !== 'all')
                            ...
                            <p class="text-xs text-gray-400 mt-1">Try adjusting your search or tab.</p>
                        @else
                            <div class="bg-green-50 p-6 rounded-full mb-4">
                                <svg class="h-10 w-10 text-green-400 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                            </div>
                            <p class="font-semibold text-gray-500 text-sm">No violations on record</p>
                            <p class="text-xs text-gray-400 mt-1 text-center px-4">You're in good standing. Keep it up!</p>
                        @endif
                    </div>
                @endforelse
            </div>
        ...
        <div class="w-full lg:w-[70%] h-[750px] bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            <livewire:layouts.violations.tenant-violation-detail :initialViolationId="$activeViolationId" />
        </div>
    </div>
</div>

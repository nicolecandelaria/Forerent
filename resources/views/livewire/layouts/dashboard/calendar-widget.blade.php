<div id="calendar-card" class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden w-full flex flex-col">
    <div class="px-3 sm:px-6 py-2 sm:py-4 border-b border-gray-100">
        <h3 class="text-sm sm:text-lg font-semibold text-[#070642]">Calendar</h3>
    </div>

    <div class="flex-1 min-h-0 flex flex-col lg:flex-row">
        {{-- Calendar Panel --}}
        <div id="calendar-grid-panel" class="lg:w-[340px] lg:min-w-[340px] p-2 sm:p-4 border-b lg:border-b-0 lg:border-r border-gray-100">
            <div class="max-w-xs sm:max-w-sm mx-auto">
                ...
                <div class="flex items-center justify-between mb-2 sm:mb-4">
                    ...
                    <h3 class="text-xs sm:text-sm font-bold text-gray-900 text-center flex-1">{{ $currentMonth }}</h3>
                ...
                <div class="grid grid-cols-7 gap-0.5 sm:gap-1 text-center">
                    ...
                    <div class="text-[10px] sm:text-xs font-medium text-gray-600 pb-0.5 sm:pb-1">{{ $day }}</div>
                    @endforeach
                    ...
                    @foreach($calendarDays as $day)
                        ...
                        @else
                            <button
                                ...
                                class="aspect-square flex flex-col items-center justify-center rounded text-[11px] sm:text-xs transition-all
        ...
        <div id="selected-date-panel" class="flex-1 min-h-0 flex flex-col overflow-hidden">
            <div class="bg-blue-700 px-3 sm:px-6 py-2 sm:py-3">
                <p class="text-[10px] sm:text-[11px] font-medium uppercase tracking-wide text-blue-100">Selected Date</p>
                <h3 class="text-white text-xs sm:text-sm font-semibold">{{ \Carbon\Carbon::parse($selectedDate)->format('F d, Y') }}</h3>
            </div>
            <div class="flex-1 min-h-0 p-4 sm:p-6 overflow-y-auto sm:pr-3">
                ...
                @empty
                    <div class="h-full flex items-center justify-center">
                        <p class="text-gray-500 text-center text-xs sm:text-sm">No announcements for selected date</p>
                    </div>
                @endforelse
            </div>
        </div>

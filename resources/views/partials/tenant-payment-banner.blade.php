        ...
        <div>
            ...
            <div class="mt-1.5 sm:mt-2.5">
                ...
                @elseif($daysUntilDue > 0)
                    <span class="text-xs sm:text-[13px] font-semibold text-blue-200">{{ round($daysUntilDue) }} {{ round($daysUntilDue) === 1 ? 'day' : 'days' }} left to pay</span>
                @elseif($daysUntilDue == 0)
                    <span class="text-xs sm:text-[13px] font-bold text-red-300 animate-pulse">Due today!</span>
                @else
                    <span class="text-xs sm:text-[13px] font-bold text-red-300">{{ abs(round($daysUntilDue)) }} {{ abs(round($daysUntilDue)) === 1 ? 'day' : 'days' }} overdue</span>
                @endif
            </div>
        </div>
        <div class="flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-2.5">
            @if($paymentStatus !== 'No Billing')
                <span class="inline-flex items-center px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider
            ...
            @if(in_array($paymentStatus, ['Unpaid', 'Overdue']) && count($pendingPaymentRequests) === 0)
                <button wire:click="openPaymentModal" class="inline-flex items-center gap-1.5 px-4 sm:px-5 py-1.5 sm:py-2 rounded-xl bg-white text-[#070589] text-xs sm:text-[13px] font-bold uppercase tracking-wide transition hover:bg-blue-50 shadow-lg shadow-black/10">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    Pay Now
                </button>
            @elseif(count($pendingPaymentRequests) > 0)
                <span class="inline-flex items-center gap-1.5 px-3 sm:px-4 py-1 sm:py-1.5 rounded-lg bg-white/10 text-blue-200 text-xs sm:text-[13px] font-bold uppercase tracking-wide ring-1 ring-white/15">
